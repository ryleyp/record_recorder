...
## Import subsystem (v1.1)

- `AudioImporter` probes files with `AVAudioFile` (format, rate, depth,
  channels, duration; bitrate estimated from file size for compressed
  formats), produces quality warnings, scans folders (junk-filtered, sorted
  by embedded ID3 track number → name → creation date), and copies sources
  into the package collision-safely. `VolumeWatcher` lists removable volumes
  via `mountedVolumeURLs` + `NSWorkspace` mount notifications.
- `RecordSide` gained `sourceType` (`liveRecording` / `importedFile` /
  `importedFolder`), source info/warnings, reference paths + bookmarks, and
  per-track imported files. Decoding is manual (`decodeIfPresent`) so v1
  project files still open. Sides resolve their playable audio through
  `audioURL(in:)`; imported audio is analyzed/played/exported directly in its
  original format — no transcoding until export.
- `TracklistParser` extracts titles/runtimes (plus Artist/Album/Year headers)
  from pasted text; `TracklistAligner` scales the runtimes to the measured
  side length and snaps each expected cut to the best-scoring nearby detected
  gap (falling back to the expected position when no gap exists).
- `AudacityLabels` parses exported label tracks (region and point labels) into
  boundaries + titles applied at import.
- `MP3Retagger` strips ID3v2/v1 and prepends fresh tags so imported MP3
  tracks export with byte-identical audio frames when "Keep original
  encoding" is on (asserted byte-for-byte in tests).
